    ...
    let method_items = item_trait
        ...
        .filter_map(|trait_item| match trait_item {
            syn::TraitItem::Method(method) => {
                Some(gen_delegation_method(method, &generic_idents, &attr))
            }
            _ => None,
        });
...
pub fn output_tokens2(
    ...
    let where_clause = ImplWhereClause2 {
        out_trait: &out_trait,
        contains_async,
...
impl<'g, 'c> quote::ToTokens for ImplWhereClause2<'g, 'c> {
    fn to_tokens(&self, stream: &mut TokenStream) {
        let mut punctuator = Punctuator::new(
            stream,
            syn::token::Where(self.span),
