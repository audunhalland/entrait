...
fn gen_trait_def(
    ...
    let opt_delegate_impl_attr = match &generics.deps {
        generics::Deps::Concrete(_) => {
            Some(quote! { #[::entrait::delegate_impl(unimock = false, mockall = false)] })
        }
        _ => None
    };
...
fn gen_impl_blocks(
    ...
    Ok(match &generics.deps {
        generics::Deps::Concrete(path) => {
            // NOTE: the impl for Impl<T> is generated by invoking #[delegate_impl] on the trait
