...
pub struct ImplWhereClause<'a, 'i, 'g> {
...
impl<'a, 'i, 'g> quote::ToTokens for ImplWhereClause<'a, 'i, 'g> {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let has_initial_clause = match &self.generics.deps {
            generics::Deps::Generic {
                trait_bounds,
                idents,
                ..
            } => {
                ...
                // T bounds
                {
                    idents.impl_t.to_tokens(tokens);
                    syn::token::Colon(self.span).to_tokens(tokens);
                    ...
                    if self.input_fn.use_associated_future(self.attr) {
                        ...
                        syn::Lifetime::new("'static", self.span).to_tokens(tokens);
                    }
                }

                true
            }
            generics::Deps::NoDeps { idents } => {
                syn::token::Where::default().to_tokens(tokens);
        ...
        match (
            has_initial_clause,
            &self.generics.trait_generics.where_clause,
        ) {
            (true, Some(where_clause)) => {
                syn::token::Comma(self.span).to_tokens(tokens);
                where_clause.predicates.to_tokens(tokens);
            }
            (false, Some(where_clause)) => {
